    ...
    for test in testToRun:
        ...
        for currentTestNo in range(0, len(paramDict["TESTS"][:]), 2):
            ...
            tempThread = threading.Thread(
                target=getTemps,
                args=(duration, 500, test, startEvent, currentTestNo)
            )
            ...
            # FIX: corrected missing slash in path for monotonic log
            subprocess.run(
                "adb pull /sdcard/Android/data/com.YourCompany.SunTemple2/filesmonotonic_log.csv",
                shell=True
            )

            pull_data_thread = threading.Thread(target=pullData, args=(test, currentTestNo, graphicalPresets[test]))
            ...
            subprocess.run("adb pull /sdcard/screen.png ../../testData/", shell=True)
